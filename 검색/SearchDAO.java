...
public class SearchDAO {
	...
	public Collection<MainVO> searchCategory(String w) {
		...
		try {
			...
			while(rs.next()) { 
				list.add(new MainVO(rs.getInt(1), rs.getString(2), rs.getString(3), null, null, 0, 0, rs.getInt(4), 
    		            rs.getString(5), rs.getString(6), rs.getString(7), null, null, rs.getFloat(8)));
			}
	...
	public Collection<MainVO> searchProduct(String w, int category2) {
		Collection<MainVO> list=new ArrayList<>();
		String word = w;
		try {
			Statement stmt = conn.createStatement(); 
	...
	public Collection<BazaarVO> searchBazaar(String w) {
		...
		try {
			Statement stmt = conn.createStatement(); 
			String sql = " select b.bazzar_num, b.name, b.img1, b.price, b.category, (CASE b.pro_condition" + 
					" WHEN '0' THEN '상' WHEN '1' THEN '중' WHEN '2' THEN '하' END)," +
			...
			while(rs.next()) { 
				list.add(new BazaarVO(rs.getInt(1), rs.getString(2), rs.getString(3), null, null, 0, 0, rs.getInt(4), 
    		            rs.getString(5), rs.getString(6), rs.getString(7), null, null, rs.getFloat(8)));
			}
			System.out.println(word);
			rs.close();
